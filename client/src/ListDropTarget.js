...
import {
  openFolder, closeFolder, moveFolder, addTree,
...
const ListTargetInner = (props) => {
  const {
    isFolder, isOver, writeEnabled, adminEnabled, openDocumentIds, item, openFolderContents, allDraggable,
  } = props;
  if (isFolder) {
    ...
    return (
      <DocumentFolder
        item={item}
        key={`${item.document_kind}-${item.id}`}
        inContents
  ...
  return (
    <div style={{ width: 'auto', height: '8px', margin: '0 8px' }}>
      <div style={{
        width: 'auto', height: '0', marginTop: '4px', borderTop: isOver ? '2px solid #666' : 'none',
      }}
      />
    </div>
  );
...
function handleFileSystemDrop(props, monitorItem) {
  const {
    addTree, targetParentId, targetParentType, buoyancyTarget,
  } = props;
  ...
  reader.onload = (e) => {
    try {
      const sequences = parseIIIFManifest(reader.result);
      if (sequences) {
        // for manifests with a single sequence, don't create an extra subfolder
        const tree = (sequences.length === 1) ? {
...
function handleDMItemDrop(props, monitorItem) {
  ...
  if (props.inContents && monitorItem.isFolder) {
    ...
    if (
      ...
      !props.items.find((item) => (monitorItem.linkable_type === 'Highlight' && item.highlight_id === monitorItem.linkable_id)
  ...
  handler(idToPass, targetID, props.buoyancyTarget)
    ...
    // TODO these shouldn't happen until we get an OK back from the server
      if (monitorItem.existingParentType === 'Project' || props.targetParentType === 'Project') props.loadProject(props.projectId);
      if (monitorItem.existingParentType === 'DocumentFolder' && props.openFolderContents[monitorItem.existingParentId]) props.openFolder(monitorItem.existingParentId);
      if (props.targetParentType === 'DocumentFolder' && props.openFolderContents[props.targetParentId]) props.openFolder(props.targetParentId);
    });
}
