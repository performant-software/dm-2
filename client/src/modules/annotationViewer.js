...
export default function (state = initialState, action) {
  switch (action.type) {
    case SELECT_TARGET:
      return {
        ...state,
        selectedTargets: state.selectedTargets.concat([{ loading: true, opened: Date.now(), ...action.target }]),
      };

    case SELECT_LOAD_SUCCESS:
      const preUpdateTargetsCopy = state.selectedTargets.slice(0);
      let toUpdateIndex = -1;
      ...
      if (toUpdateIndex >= 0) {
        const updatedTarget = { opened: Date.now(), ...action.target };
    ...
    case REFRESH_TARGET:
      const preRefreshTargetsCopy = state.selectedTargets.slice(0);
      if (action.index >= 0) {
        const refreshingTarget = { ...preRefreshTargetsCopy[action.index] };
    ...
    case REFRESH_SUCCESS:
      const preReplaceTargetsCopy = state.selectedTargets.slice(0);
      if (action.index >= 0) {
        const refreshedTarget = { ...state.selectedTargets[action.index] };
    ...
    case SELECT_SIDEBAR_LOAD_SUCCESS:
      const updatedTarget = { ...action.target };
      Object.assign(updatedTarget, action.loadedTarget);
      return {
        ...state,
    ...
    case CLOSE_TARGET: {
      const preCloseTargetsCopy = state.selectedTargets.slice(0);
      let toCloseIndex = -1;
    ...
    case PROMOTE_TARGET:
      const prePromoteTargetsCopy = state.selectedTargets.slice(0);
    ...
    case DELETE_LINK_SUCCESS:
      ...
      let nextSidebarTarget = null; let
        nextSelectedTargets;
      if (sidebarTarget) {
...
export function refreshTargetByDocumentID(document_id) {
  return function (dispatch, getState) {
    const { selectedTargets } = getState().annotationViewer;
  ...
  return function (dispatch, getState) {
    fetch('/links', {
      ...
      .then((newLink) => {
        ...
        if (sidebarTarget) {
          const { highlight_id, document_id } = sidebarTarget;
          if (highlight_id) {
            if ((origin.linkable_type === 'Highlight' && origin.linkable_id === highlight_id) || (linked.linkable_type === 'Highlight' && linked.linkable_id === highlight_id)) dispatch(selectSidebarTarget(sidebarTarget));
          } else if ((origin.linkable_type === 'Document' && origin.linkable_id === document_id) || (linked.linkable_type === 'Document' && linked.linkable_id === document_id)) dispatch(selectSidebarTarget(sidebarTarget));
        }
        getState().annotationViewer.selectedTargets.forEach((target, index) => {
          const { highlight_id, document_id } = target;
          if (highlight_id) {
            if ((origin.linkable_type === 'Highlight' && origin.linkable_id === highlight_id) || (linked.linkable_type === 'Highlight' && linked.linkable_id === highlight_id)) dispatch(refreshTarget(index));
          } else if ((origin.linkable_type === 'Document' && origin.linkable_id === document_id) || (linked.linkable_type === 'Document' && linked.linkable_id === document_id)) dispatch(refreshTarget(index));
        });
        if (newLinkPosition) {
          dispatch({
